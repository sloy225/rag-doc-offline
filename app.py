...
# Fichier de log
# -----------------------------
log_file = DATA_DIR / "evaluation_log.csv"
...
if uploaded_files:
    ...
    st.success(
        f"{len(uploaded_files)} fichier(s) ajouté(s)."
    )
...
if st.button(
    ...
    with st.spinner("Indexation..."):
        ...
        total_chunks, infos = build_vectorstore(
            embed_model
        )
    ...
    if total_chunks == 0:
        ...
        st.success(
            f"Index créé ({total_chunks} chunks)."
        )
...
if st.button("Interroger l'agent"):
    ...
    if not question.strip():
        ...
        if result is None:
            ...
            st.error(
                "Impossible d'exécuter la demande."
            )
            ...
            if action == "rag":
                ...
                st.info(
                    "🔎 Recherche documentaire FAISS + génération LLM"
                )

            elif action == "list":

                st.info(
                    "📂 Consultation du corpus local"
                )
